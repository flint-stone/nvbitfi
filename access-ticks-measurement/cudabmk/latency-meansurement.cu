...
__global__ void global_latency (ptrsize_type** my_array, int array_length, int iterations, unsigned long long * duration) {
    ...
    for (int k = 0; k < iterations; k++)
    {
        ...
        for (int x = 0; x < 1024; x++){
            j=*(ptrsize_type **)j;
            printf("accessed pointer %lli next %lli \n", (unsigned long long int)j, *(ptrsize_type **)j);
        }
        end_time = clock64();
...
void measure_global_latency()
{
    ...
    for (int stride = 2; stride <= 1024; stride+=1)
    {
    ...
    // for (int stride = 1024*1024; stride >= 1024; stride-=1024)
    // {
