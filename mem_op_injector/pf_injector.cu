...
void instrument_function_if_needed(CUcontext ctx, CUfunction func) {
		...
		for (auto f : related_functions) {
				...
				for(auto i: instrs)  {
						...
						if (verbose) printf("extracted instType: %s, isLoad %d isStore %d getMemOpBytes %d ", instTypeStr.c_str(),  i->isLoad(), i->isStore(), i->getSize());
						...
						if ((uint32_t)instType == inj_info.injInstType || inj_info.injInstType == NUM_ISA_INSTRUCTIONS) {
								...
								if (tokens.size() > 1) { // an actual instruction that writes to either a GPR or PR register
										...
										if (regtype == 0) { // GPR reg
												...
												nvbit_insert_call(i, "print_op", IPOINT_AFTER);
		...
		if (cbid == API_CUDA_cuLaunch ||
				...
				if(!is_exit) {
						...
						if (kernel_id < limit) {
								...
								std::cout << "clock returned: " << inj_info.clock << std::endl;
